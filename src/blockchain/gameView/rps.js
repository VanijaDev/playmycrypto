...
const RPS = {
  ...
  setPlaceholders: function () {
    ...
    $('#rpsstart_seed')[0].placeholder = $t.seed_str_placeholder;
    ...
    $('#rpscreatormove_previous_move_seed')[0].placeholder = $t.str_from_prev_move;
    $('#rpscreatormove_next_move_seed')[0].placeholder = $t.seed_str_placeholder;
  },
  ...
  increaseBetClicked: async function () {
    ...
    if ((bet.length == 0) || (new BigNumber(Utils.etherToWei(bet)).comparedTo(this.minBet) < 0)) {
      let str = $tx.err_bet_increase_min + Utils.weiToEtherFixed(this.minBet, 2) + " " + window.BlockchainManager.currentCryptoName();
      showTopBannerMessage(str, null, true);
      return;
    }
  ...
  playMoveClicked: async function () {
    ...
    if (this.skipNextMove) {
      ...
      if (this.selectedMove == 0) {
        showTopBannerMessage($t.select_next_move, null, true);
        return;
      }

      if (seedStr.length == 0) {
        showTopBannerMessage($t.enter_next_seed, null, true);
        return;
      }
    }
  ...
  makeMoveClicked: async function () {
    console.log('%c makeMoveClicked', 'color: #e51dff');
    ...
    window.BlockchainManager.gameInst(Types.Game.rps).methods.opponentNextMove(gameId, this.selectedMove).send({
      ...
      .once('error', function (error, receipt) {
        ...
        if (error.code != window.BlockchainManager.MetaMaskCodes.userDenied) {
          showTopBannerMessage($t.err_make_move, null, true);
          throw new Error(error, receipt);
        }
      });
  },
